...
class TransformControls extends THREE.Object3D {
  ...
  pointerHover(pointer) {
    ...
    let objects = [];
    objects = objects.concat(this._gizmo.picker["translate"]);
    objects = objects.concat(this._gizmo.picker["rotate"]);
    objects = objects.concat(this._gizmo.picker["scale"]);
    // objects = objects.concat( this._gizmo.gizmo[ 'translate' ] );
    // objects = objects.concat( this._gizmo.gizmo[ 'rotate' ] );
    // objects = objects.concat( this._gizmo.gizmo[ 'scale' ] );

    const intersect = intersectObjectWithRay(objects, _raycaster);

    if (intersect) {
      this.axis = intersect.object.name;
      if (this.axis.startsWith("R")) {
        this.mode = "rotate";
      } else if (this.axis.startsWith("S")) {
        this.mode = "scale";
      } else {
        this.mode = "translate";
      }
    ...
    if (mode === "translate") {
      ...
      if (this.rotationSnap)
        this.rotationAngle =
          ...
          this.rotationSnap; // Apply rotate
  ...
  setTranslateEnabled(enable) {
    this._gizmo.translateEnabled = enable;
  }
  setRotateEnabled(enable) {
    this._gizmo.rotateEnabled = enable;
  }
  setScaleEnabled(enable) {
    this._gizmo.scaleEnabled = enable;
  }
...
function intersectObjectWithRay(object, raycaster, includeInvisible) {
  const allIntersections =
    object instanceof Array
      ? raycaster.intersectObjects(object, true)
...
class TransformControlsGizmo extends THREE.Object3D {
  constructor() {
    ...
    const arrowGeometry = new THREE.CylinderGeometry(0, 0.03, 0.08, 12);
    arrowGeometry.translate(0, 0.04, 0);
    const scaleHandleGeometry = new THREE.BoxGeometry(0.04, 0.04, 0.04);
    ...
    function CircleGeometry(radius, arc, tube = 0.0075) {
    ...
    const gizmoTranslate = {
      ...
      TY: [
        [new THREE.Mesh(arrowGeometry, matGreen), [0, 0.45, 0]],
    ...
    const pickerTranslate = {
      TX: [
        [
          new THREE.Mesh(arrowGeometry, matInvisible.clone()),
          [0.45, 0, 0],
          [0, 0, -Math.PI / 2],
        ],
        [
          new THREE.Mesh(lineGeometry2, matInvisible),
          [0, 0, 0],
          [0, 0, -Math.PI / 2],
        ],
      ],
      TY: [
        [new THREE.Mesh(arrowGeometry, matInvisible.clone()), [0, 0.45, 0]],
        [new THREE.Mesh(lineGeometry2, matInvisible.clone())],
      ],
      TZ: [
        [
          new THREE.Mesh(arrowGeometry, matInvisible.clone()),
    ...
    const gizmoRotate = {
      RX: [[new THREE.Mesh(CircleGeometry(0.35, 0.5), matRed.clone())]],
      RY: [
        [
          new THREE.Mesh(CircleGeometry(0.35, 0.5), matGreen.clone()),
          null,
          [0, 0, -Math.PI / 2],
        ],
      ],
      RZ: [
        [
          new THREE.Mesh(CircleGeometry(0.35, 0.5), matBlue.clone()),
          null,
          [0, Math.PI / 2, 0],
        ],
      ],
    ...
    const pickerRotate = {
      RX: [
        [
          new THREE.Mesh(
            CircleGeometry(0.35, 0.5, 0.0175),
            matInvisible.clone()
          ),
        ],
      ],
    ...
    const gizmoScale = {
      SX: [
        [
          new THREE.Mesh(scaleHandleGeometry, matRed.clone()),
          [0.35, 0, 0],
          [0, 0, -Math.PI / 2],
        ],
      ],
      SY: [
        [new THREE.Mesh(scaleHandleGeometry, matGreen.clone()), [0, 0.35, 0]],
      ],
      SZ: [
        [
          new THREE.Mesh(scaleHandleGeometry, matBlue.clone()),
          [0, 0, 0.35],
          [Math.PI / 2, 0, 0],
        ],
      ],
    ...
    const pickerScale = {
      SX: [
        [
          new THREE.Mesh(scaleHandleGeometry, matInvisible.clone()),
          [0.35, 0, 0],
          [0, 0, -Math.PI / 2],
        ],
      ],
  ...
  updateMatrixWorld(force) {
    ...
    let quaternion =
      space === "local" ? this.worldQuaternion : _identityQuaternion; // Show only gizmos for current transform mode
    quaternion = _identityQuaternion;

    this.gizmo["translate"].visible = this.translateEnabled;
    this.gizmo["rotate"].visible = this.rotateEnabled;
    this.gizmo["scale"].visible = this.scaleEnabled;
    this.helper["translate"].visible =
      this.translateEnabled && this.mode === "translate";
    this.helper["rotate"].visible =
      this.rotateEnabled && this.mode === "rotate";
    this.helper["scale"].visible = this.scaleEnabled && this.mode === "scale";
    this.picker["translate"].visible = this.translateEnabled;
    this.picker["rotate"].visible = this.rotateEnabled;
    this.picker["scale"].visible = this.scaleEnabled;
    let handles = [];
    ...
    for (let i = 0; i < handles.length; i++) {
      ...
      if (handle.tag === "helper") {
        ...
        if (handle.name === "AXIS") {
          ...
          if (this.axis && this.axis.indexOf("X") != -1) {
          ...
          if (this.axis && this.axis.indexOf("Y") != -1) {
          ...
          if (this.axis && this.axis.indexOf("Z") != -1) {
      ...
      if (this.mode === "translate" || this.mode === "scale") {
        ...
        if (handle.name === "TX" || handle.name === "SX") {
          if (
            Math.abs(
              _alignVector
                .copy(_unitX)
        ...
        if (handle.name === "TY" || handle.name === "SY") {
          if (
            Math.abs(
              _alignVector
                .copy(_unitY)
        ...
        if (handle.name === "TZ" || handle.name === "SZ") {
          if (
            Math.abs(
              _alignVector
                .copy(_unitZ)
      ...
      handle.visible =
        ...
        (handle.name.indexOf("E") === -1 ||
          (this.showX && this.showY && this.showZ)); // highlight selected axis
